...
class Camera(object):
	...
	def Orbit(self, center, distance, angleHorizontal, angleVertical=0):
		# Convertir ángulos a radianes
		hRad = radians(angleHorizontal)
		vRad = radians(angleVertical)
		...
		self.position.x = center.x + cos(vRad) * sin(hRad) * distance
		self.position.y = center.y + sin(vRad) * distance
		self.position.z = center.z + cos(vRad) * cos(hRad) * distance